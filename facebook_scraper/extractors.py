    ...
    def extract_photo_link(self) -> PartialPost:
        ...
        for link in photo_links:
            ...
            try:
                ...
                try:
                    response.html.render(send_cookies_session=True, sleep=2)
                    # It only works the second time. No idea why.
                    response = self.request(self.post['post_url'].replace('m.facebook.com', 'www.facebook.com'), desktop_user_agent=True)
                    response.html.render(send_cookies_session=True, sleep=2)
                except Exception as e:
                    print(e)
                ...
                search_str2 = '"nextMediaAfterNodeId":{"__typename":"Video","id":"'
                next_image_id = ''
                search_2 = False
                try:
                    found_index = image_response.text.find(search_str)
                    if found_index == -1:
                        found_index = image_response.text.find(search_str2)
                        if found_index == -1:
                            raise ValueError('Next image id not found')
                        search_2 = True
                    border_left = found_index + len(search_str2 if search_2 else search_str)
                ...
                while next_image_id and next_image_id != first_image_id:
                    try:
                        image_response = self.request(image_link.replace(first_image_id, next_image_id), desktop_user_agent=True)
                        images.append(self.extract_photo_link_HQ(image_response.text))
